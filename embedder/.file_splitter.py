...
from File_to_topic import split_into_topics
...
DEFAULT_CHUNK_SIZE = 1500      # max tokens per chunk
...
def process_topic_text(
    ...
    # 8) Optional merge of tiny sibling chunks (only in "embed" mode)
...
def unified_main():
    parser = argparse.ArgumentParser(
        description="Split a Markdown file into topics (TF-IDF), then recursively chunk each topic with metadata."
    )
    parser.add_argument(
        "file_path",
    ...
    # 2) Split into topics via TF-IDF semantic merge
    ...
    # 3) For each topic subtext, run recursive chunker + metadata, writing final files
    os.makedirs(args.output_dir, exist_ok=True)
    written_files = 0

    for topic_slug, topic_text in topics:
        # Create a submenu directory per topic (optional)
        topic_dir = os.path.join(args.output_dir, topic_slug)
        if args.mode == "md":
            os.makedirs(topic_dir, exist_ok=True)

        # Process this topic’s text
        final_chunks = process_topic_text(
            topic_slug=topic_slug,
            topic_text=topic_text,
            original_filename=args.file_path,
            mode=args.mode,
        ...
        if args.mode == "md":
            # process_topic_text wrote files under topic_dir
            written_files += len(os.listdir(topic_dir))
        else:
            # In embed mode, just count how many were returned
            written_files += len(final_chunks)

    if args.mode == "md":
        print(f"Final chunked Markdown files written under '{args.output_dir}'. Total files: {written_files}")
    else:
        print(f"Total embedded chunks returned: {written_files}")
